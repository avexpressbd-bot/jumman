...
export default function AdvisoryCommittee() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const querySnapshot = await getDocs(query(collection(db, "advisory_committee"), orderBy("orderIndex", "asc")));
        const dbMembers = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as CommitteeMember));
        setMembers(dbMembers);
      } catch (err) {
        console.error("Error fetching advisory committee:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="py-20 bg-stone-50 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16 relative">
          <h1 className="text-4xl font-bold text-emerald-900 mb-4">উপদেষ্টা কমিটি</h1>
          <p className="text-emerald-800/60 max-w-2xl mx-auto">
            বিষ্ণুপুর ইউনিয়ন সোসাইটির সম্মানিত উপদেষ্টা মন্ডলী যাদের সুচিন্তিত পরামর্শে সংগঠন এগিয়ে যাচ্ছে।
          </p>
            ...
            {members.length > 0 ? members.map((member, idx) => (
              ...
              >
                <div className="aspect-square overflow-hidden relative">
                  <img
                    src={member.imageUrl || "https://picsum.photos/seed/avatar/400/400"}
                    alt={member.name}
              ...
              <div className="col-span-full text-center py-20 bg-white rounded-[3rem] border border-dashed border-emerald-200">
                <p className="text-emerald-800/40 italic">বর্তমানে কোনো উপদেষ্টা তালিকাভুক্ত নেই।</p>
              </div>
            )}
          </div>
        )}
